...
#
###############################################################################
from __future__ import (absolute_import, division, print_function, unicode_literals) # 确保代码在 Python 2 和 Python 3 中表现一致
...
# 从 Backtrader 框架中导入的，用于提供交易框架相关的功能，如时间框架处理、元编程、工具类
from mini_wt.main.backtrader import TimeFrame, Position
from mini_wt.main.backtrader.metabase import MetaParams
from mini_wt.main.backtrader.utils.py3 import bytes, bstr, queue, with_metaclass, long
from mini_wt.main.backtrader.utils import AutoDict, UTC

# bisect 是一个用于处理排序列表的二分查找算法库。
import bisect

# 兼容性处理: 确保 bytes 类型的正确使用。
bytes = bstr  # py2/3 need for ibpy

# 这些导入是 IB 的 Python API 组件，用于建立与 IB 服务器的连接（EClient）、处理服务器响应（EWrapper）、定义交易合约（Contract）和处理市场数据（TickTypeEnum）。
from ibapi.client import EClient # 服务器连接: EClient 是 Interactive Brokers (IB) 的 API 中的一个关键组件，它负责与 IB 服务器建立和管理网络连接。通过这个类，你可以发送各种类型的请求到 IB 服务器，包括但不限于市场数据请求、订单提交、账户信息请求等。它是客户端功能的实现，用于向 IB 服务器发送命令和请求。
from ibapi.wrapper import EWrapper # 服务器响应:EWrapper 是一个抽象类，用于处理从 IB 服务器返回的各种响应和数据。当你发送请求给 IB 服务器（通过 EClient），服务器会返回响应，EWrapper 的方法将被调用以处理这些响应。这包括市场数据更新、订单状态更新、账户通知等。你通常需要创建一个继承自 EWrapper 的子类，并实现相应的方法来处理所需的响应数据。
from ibapi.contract import Contract # 定义交易合约:Contract 类在 IB API 中用于定义一个交易合约。一个交易合约包括了足够的信息来唯一标识市场上的金融产品，例如股票、期权、期货、外汇等。这些信息包括合约的标识符（如股票代码）、交易所、货币、合约类型等。通过创建 Contract 对象并设置适当的属性，你可以指定你想要交易或查询的具体金融产品。
from ibapi.ticktype import TickTypeEnum # 处理市场数据: 当你从 IB 服务器接收市场数据（如价格更新、成交量等）时，每个数据点都有一个与之相关的“tick type”。这个枚举定义了所有可能的 tick types，使你能够理解接收到的市场数据代表什么。例如，它可以告诉你收到的是买价、卖价、最后成交价、成交量等。

# 这部分设置了日志记录器，用于记录交易过程中的事件、错误或其他重要信息。ENABLE_DEBUG 变量可能用于控制调试输出的开启和关闭。
import logging
logger = logging.getLogger(__name__)
ENABLE_DEBUG=True

def _ts2dt(tstamp=None):
    # 将 Interactive Brokers (IB) API 中的时间戳转换为 Python datetime 对象。
    ...
    usec = msec * 1000
    return dtm.utcfromtimestamp(sec).replace(microsecond=usec)
...
class RTVolume(object):
    ...
    constituent fields
    这个 RTVolume 类用于解析 Interactive Brokers (IB) API 中 tickString 事件类型为 48（即实时成交量 RTVolume）的数据。
    Supports using a "price" to simulate an RTVolume from a tickPrice event
    '''
    _fields = [
        ('price', float), # 成交价格
        ('size', float), # 单笔成交量
        ('datetime', _ts2dt), # 日期时间
        ('volume', float), # 代表在此特定时间点的累积成交量。这是从当日开始到当前时间的总成交量。
        ('vwap', float), # 代表成交量加权平均价格（Volume Weighted Average Price）。这是一个用成交量加权计算的平均价格，用于评估价格的标准水平。
        ('single', bool) # 这个字段标志着该成交记录是否是单一成交。在某些情况下，一个大的订单可能会被分解成多个小的成交，而这个标志用于指示当前记录是否只是一个单一的成交。
    ]
...
class RTPrice(object):
    ...
    def __init__(self, price, tmoffset=None):
        ...
        self.datetime = dtm.now()

        if tmoffset is not None:
            self.datetime += tmoffset # tmoffset 是一个时间偏移量，用于调整 RTVolume 数据中的时间信息。它通常是一个 datetime.timedelta 对象，表示要加到原始时间上的时间量。
...
class RTSize(object):
    ...
    def __init__(self, size, tmoffset=None):
        ...
        if tmoffset is not None:
            self.datetime += tmoffset
    def __str__(self):
        return f"Size: {self.size}, DateTime: {self.datetime}"

class RTBar(object): # 封装了一根 K 线的所有基本信息
    '''Set realtimeBar object
    '''
    def __init__(self, reqId, time, open_, high, low, close_, volume, wap, count):
        self.vars = vars()
        self.reqId = reqId
        self.time = time
        self.open = open_
...
class HistTick(object):
    ...
    def __init__(self, tick, dataType):
        self.vars = vars()
        self.date = dtm.utcfromtimestamp(tick.time)
        self.tickType = tick.tickType if hasattr(tick, 'tickType') else int(0)
        self.dataType = dataType # dataType 指定了Tick数据的类型，如“中点”（Midpoint）、“买卖价”（Bid/Ask）或“交易”（Trades）
        if dataType == 'RT_TICK_MIDPOINT': # 对于 'RT_TICK_MIDPOINT' 数据类型，只存储价格。
            self.price = tick.price
        elif dataType == 'RT_TICK_LAST': # 对于 'RT_TICK_LAST' 数据类型，存储价格、大小、未报告的交易标志和过去限价标志。
            self.price = tick.price
            self.size = float(tick.size)
            self.unreported = tick.tickAttribLast.unreported
            self.pastlimit = tick.tickAttribLast.pastLimit
        elif dataType == 'RT_TICK_BID_ASK': # 对于 'RT_TICK_BID_ASK' 数据类型，存储买价、卖价、买量和卖量。
...
class RTTickLast(object):
    ...
    def __init__(self, tickType, time, price, size, tickAtrribLast, exchange, specialConditions):
        ...
        self.datetime = dtm.utcfromtimestamp(time)
...
class IBApi(EWrapper, EClient):
    ...
    def tickPrice(self, reqId, tickType, price, attrib):
        ...
        # print("Tick Price - Ticker ID:", reqId, "Tick Type:", tickType, "Price:", price)
        # print(f"tickPrice - Ticker ID: {reqId}, Tick Type: {tickType}, Price: {price}")
    ...
    def realtimeBar(self, reqId, time, open_, high, low, close, volume, wap, count):
        #print("Realtime Bar is Executing - Req ID:", reqId, "Time:", time, "Open:", open, "High:", high, "Low:", low, "Close:", close)
        self.cb.realtimeBar(RTBar(reqId, time, open_, high, low, close, float(volume), wap, count))

    @logibmsg
    def historicalData(self, reqId, bar):
        # print(
          #   f"Historical Data - ReqID: {reqId}, Date: {bar.date}, Open: {bar.open}, High: {bar.high}, Low: {bar.low}, Close: {bar.close}, Volume: {bar.volume}")
...
class IBStore(with_metaclass(MetaSingleton, object)):
    ...
    Params:
      ...
      - ``timeoffset`` (default: ``True``)
        ...
        The time offset will propagate to other parts of the ``bt``
    ...
    def currentTime(self, time):
        if not self.p.timeoffset:  # only if requested ... apply timeoffset
            return
        curtime = dtm.fromtimestamp(float(time))
        with self._lock_tmoffset:
            self.tmoffset = curtime - dtm.now()
    ...
    def reqHistoricalDataEx(self, contract, enddate, begindate,
        ...
        self.conn.reqHistoricalData(
            ...
            bytes(intdate.strftime('%Y%m%d %H:%M:%S')+ ' US/Eastern'),
            bytes(duration),
            bytes(barsize),
            bytes(what),
            int(useRTH),
    ...
    def reqHistoricalTicksEx(self, contract, enddate=None, begindate=None,
        ...
        if enddate is None and begindate is None:
            ...
            begindate = dtm(today.year, today.month, today.day)
    ...
    def reqRealTimeBars(self, contract, useRTH=False, duration=5, what = None):
        # print("Requesting Real Time Bars for:", contract.symbol, "Duration:", duration, "What:", what)
    ...
    def reqMktData(self, contract, what=None):
        #print("啊啊啊啊啊啊啊啊啊啊！！！！reqMktData is running!!!")
    ...
    def realtimeBar(self, msg):
        ...
        msg.time = dtm.utcfromtimestamp(float(msg.time))
        self.qs[msg.reqId].put(msg)
        # print(f"Realtime Bar - Req ID: {msg.reqId}, Time: {msg.time}, Close: {msg.close}")
    ...
    def historicalData(self, msg):
        ...
        # print(f"Before-parsing Historical Data - Req ID: {msg.reqId}, Date: {msg.date}, Close: {msg.close}")
        tickerId = msg.reqId
        q = self.qs[tickerId]
        ...
        if self.histfmt[tickerId]:
            ...
            dt = dtm.strptime(dtstr, '%Y%m%d')
            dteos = dtm.combine(dt, sessionend)
            ...
            if dteosutc <= dtm.utcnow():
                dt = dteosutc

            msg.date = dt
        else:
            msg.date = dtm.utcfromtimestamp(long(dtstr))
        # print(f"After parsing Historical Data - Req ID: {msg.reqId}, Date: {msg.date}, Close: {msg.close}")
        q.put(msg)
