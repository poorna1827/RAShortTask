...
class TestStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.csvfile = open(r'C:\FinTech\wt_data\time_diff\time_and_order_measure_20240528_2.csv', 'w', newline='')
        self.csvwriter = csv.writer(self.csvfile)

        # Write headers to the CSV file
        self.csvheaders = ['local_time', 'datetime', 'time_diff', 'open', 'high', 'low', 'close',
                           'volume', 'openinterest', 'ma', 'order_created', 'executed_time',
                           'execution_time_diff', 'executed_price', 'executed_value', 'executed_comm']
    ...
    def notify_order(self, order):
        if order.status in [order.Submitted, order.Accepted]:
            # Capture the order creation time
            self.order_created_time = dt.now().astimezone(pytz.timezone('US/Eastern'))
            return

        if order.status in [order.Completed]:
            # Store order execution details
            self.executed_time = bt.num2date(order.executed.dt)
            self.execution_time_diff = (
                        self.executed_time - self.order_created_time).total_seconds() if self.order_created_time else None
            self.executed_price = order.executed.price
            self.executed_value = order.executed.value
            self.executed_comm = order.executed.comm
    ...
    def next(self, frompre=False):
        ...
        # print(f"Yutong Zhao Updating at next() ==> Open: {data.open[0]}, High: {data.high[0]}, Low: {data.low[0]}, Close: {data.close[0]}, Volume: {data.volume[0]}, OpenInterest: {data.openinterest[0]}")
        # 指标值
        # 布林带上轨
        ...
        if self.datastatus > 0:
            ...
            row = [
                ...
                self.sma[0] if self.sma else 'nan',
                self.order_created_time,
                self.executed_time,
                self.execution_time_diff,
                self.executed_price,
                self.executed_value,
            ...
            # Flush the file to write data immediately
            self.csvfile.flush()
...
def runstrategy(args, tickers):
    ...
    tf1 = args.timeframe1 if args.timeframe1 is not None else timeframe
    cp1 = args.compression1 if args.compression1 is not None else args.compression
    ...
    print("Data fetching parameters:", datakwargs)
    ...
    for ticker in tickers:
        ...
        data = IBDataFactory(dataname=ticker, **datakwargs)
        data._name = ticker  # Give a name to data feed for logging purposes
        if args.resample:
            cerebro.resampledata(data, **rekwargs)
        elif args.replay:
            cerebro.replaydata(data, **rekwargs)
        else:
...
if __name__ == '__main__':
    ...
    selected_stocks = pd.read_csv(r"C:\FinTech\wt_data\selected_sp_100_ticker_list.txt",
                                  names=['tickers'], header=None)['tickers'].to_list()[0:10]
