...
from mini_wt.main.backtrader.utils.py3 import (map, with_metaclass, string_types,
                                  integer_types)
...
class WriterFile(WriterBase):
    ...
    #             kline += ' ' + str(val)
    #             self.writeline(kline)
    def writedict(self, dct, level=0, recurse=False, max_depth=10, seen=None):
        if seen is None:
            seen = set()

        if id(dct) in seen:
            self.writeline(' ' * (level * self.p.indent) + '... (cyclic reference detected)')
            return  # Avoid infinite recursion due to cyclic references

        seen.add(id(dct))

        if level > max_depth:
            self.writeline(' ' * (level * self.p.indent) + '... (max depth reached)')
            return
        ...
        for key, val in dct.items():
            ...
            if sclass:
                ...
                if recurse:
                    self.writelineseparator(level=level)
                self.writeline(kline)
                self.writedict(val, level=level + 1, recurse=True, max_depth=max_depth, seen=seen)
            elif isinstance(val, (list, tuple, collections.Iterable)):
                line = ', '.join(map(str, val))
                self.writeline(kline + ' ' + line)
            else:
